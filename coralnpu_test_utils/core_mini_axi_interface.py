...
class CoreMiniAxiInterface:
                ...
                try:
                    ...
                    if val.is_resolvable:
                        ...
                        nonx_data = val.binstr.replace("X",
                                                       "0").replace("z", "0")
                        rdata["data"] = np.array([
                            int(nonx_data[i:i + 8], 2)
                            for i in range(0, len(nonx_data), 8)
                        ],
                                                 dtype=np.uint8)
        ...
        for segment in elf_file.iter_segments(type="PT_LOAD"):
            ...
            if self._axi_memory_contains(header["p_paddr"]) and \
               self._axi_memory_contains(header["p_paddr"] + len(data) - 1):
                ...
                continue
            self.dut._log.info(
                f"Backdoor loading {len(data)} bytes to 0x{header['p_paddr']:08x}"
            )
    ...
    def write_memory(self, wdata):
        ...
        data = wdata["data"]
        strb = wdata["strb"]
        if addr < self.memory_base_addr or addr >= (self.memory_base_addr +
                                                    len(self.memory)):
            return False
